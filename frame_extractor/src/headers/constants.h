 ...
 *
 * $Author: ELAMIHA $
 * $Date: 2014-03-10 19:55:50 +0100 (Mo, 10 Mrz 2014) $
 * $Revision: 410 $
 * @remarks              
 * 
 */
...
#define  fHIGHBYTE(x)   ( (( x ) & 0xFF00) >> 8 )       //!< @todo 
#define  fLOWBYTE(x)    ( ( x ) & 0x00FF )              //!< @todo

#define  fHIGHWORD(x)   ( (( x ) & 0xFFFF0000) >> 16 )  //!< @todo
#define  fLOWWORD(x)    ( ( x ) & 0x0000FFFF )          //!< @todo
...
/**
 * \def A_UTILS_DEPRECATED(_func_)
 * 
 *   Mark a function or variable as deprecated.
 *
 *   To mark  the function   \code void foo(...); \endcode as deprecated declare
 *   \code void A_UTILS_DEPRECATED(foo)(...); \endcode
 *
 *   @param [in] _func_ The function or variable
 *
 */

#ifdef A_UTILS_NO_DEPRECATED_WARNING
   #define A_UTILS_DEPRECATED(_func_) _func_
#else
   #ifndef WIN32 
       #define A_UTILS_DEPRECATED(_func_) __attribute__ ((deprecated)) _func_
   #else
      #define A_UTILS_DEPRECATED(_func_) __declspec( deprecated("This is deprecated. See documentation for further information!")) _func_
   #endif
#endif

#ifdef A_UTILS_LEGACY_CONSTANTS_MACROS
    #include "constantslegacy.h"
#endif //A_UTILS_LEGACY_CONSTANTS_MACROS
