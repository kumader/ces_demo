...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class BaseBoardTempReg 
...
BaseBoardTempReg::BaseBoardTempReg ( ) 
: its0Addr ( 0x10d )
, its0Reg ( 0 )
...
BaseBoardTempReg::BaseBoardTempReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool BaseBoardTempReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x10d) return false; //init value; on false, do not update aByteOffset
...
bool BaseBoardTempReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x10d) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void BaseBoardTempReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void BaseBoardTempReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_BaseBoardTempReg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "BaseBoardTemp_deg100" );
    aLogFormat . LogDecValue( aStreamRef, its0BaseBoardTemp_deg100 );
