...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class CalibStatus2Reg 
...
CalibStatus2Reg::CalibStatus2Reg ( ) 
: its0Addr ( 0x118 )
, its0Reg ( 0 )
...
CalibStatus2Reg::CalibStatus2Reg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool CalibStatus2Reg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x118) return false; //init value; on false, do not update aByteOffset
...
bool CalibStatus2Reg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x118) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void CalibStatus2Reg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void CalibStatus2Reg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_CalibStatus2Reg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "NoWigglingCalibration" );
    aLogFormat . LogDecValue( aStreamRef, its0NoWigglingCalibration );
