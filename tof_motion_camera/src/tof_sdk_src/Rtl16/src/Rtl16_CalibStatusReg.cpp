...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class CalibStatusReg 
...
CalibStatusReg::CalibStatusReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool CalibStatusReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x21) return false; //init value; on false, do not update aByteOffset
...
bool CalibStatusReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x21) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void CalibStatusReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void CalibStatusReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_CalibStatusReg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "Status" );
    aLogFormat . LogDecValue( aStreamRef, its0Status );
    aLogFormat . LogElement( aStreamRef, "ErrorOccured" );
    aLogFormat . LogDecValue( aStreamRef, its10ErrorOccured );
    aLogFormat . LogElement( aStreamRef, "NoFPPNCalibrationDataInNVRAM" );
    aLogFormat . LogDecValue( aStreamRef, its12NoFPPNCalibrationDataInNVRAM );
    aLogFormat . LogElement( aStreamRef, "NoLensCalibrationDataInNVRAM" );
    aLogFormat . LogDecValue( aStreamRef, its14NoLensCalibrationDataInNVRAM );
