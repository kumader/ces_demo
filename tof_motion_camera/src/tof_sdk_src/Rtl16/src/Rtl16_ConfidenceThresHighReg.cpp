...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class ConfidenceThresHighReg 
...
//------------------------------ Includes ------------------------------
#include "stdafx.h"
#include "Rtl16_ConfidenceThresHighReg.h"
...
ConfidenceThresHighReg::ConfidenceThresHighReg ( ) 
: its0Addr ( 0x11 )
, its0Reg ( 0xffff )
...
ConfidenceThresHighReg::ConfidenceThresHighReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool ConfidenceThresHighReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{
...
bool ConfidenceThresHighReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x11) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void ConfidenceThresHighReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void ConfidenceThresHighReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_ConfidenceThresHighReg" );
