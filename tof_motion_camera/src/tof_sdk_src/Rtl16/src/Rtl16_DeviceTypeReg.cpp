...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class DeviceTypeReg 
...
DeviceTypeReg::DeviceTypeReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool DeviceTypeReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x6) return false; //init value; on false, do not update aByteOffset
...
bool DeviceTypeReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x6) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void DeviceTypeReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void DeviceTypeReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_DeviceTypeReg" );
    ...
    aLogFormat . LogHexValue( aStreamRef, its0HardwareSpecificIdentification );
