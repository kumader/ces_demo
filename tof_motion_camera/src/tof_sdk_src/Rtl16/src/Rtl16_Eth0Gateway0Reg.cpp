...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class Eth0Gateway0Reg 
...
Eth0Gateway0Reg::Eth0Gateway0Reg ( ) 
: its0Addr ( 0x248 )
, its0Reg ( 0x1 )
...
Eth0Gateway0Reg::Eth0Gateway0Reg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool Eth0Gateway0Reg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x248) return false; //init value; on false, do not update aByteOffset
...
bool Eth0Gateway0Reg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x248) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void Eth0Gateway0Reg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void Eth0Gateway0Reg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_Eth0Gateway0Reg" );
