...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class Eth0Snm1Reg 
...
Eth0Snm1Reg::Eth0Snm1Reg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool Eth0Snm1Reg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x247) return false; //init value; on false, do not update aByteOffset
...
bool Eth0Snm1Reg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x247) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void Eth0Snm1Reg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void Eth0Snm1Reg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_Eth0Snm1Reg" );
