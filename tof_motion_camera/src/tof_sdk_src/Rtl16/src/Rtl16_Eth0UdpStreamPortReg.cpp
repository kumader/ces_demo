...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class Eth0UdpStreamPortReg 
...
Eth0UdpStreamPortReg::Eth0UdpStreamPortReg ( ) 
: its0Addr ( 0x24e )
, its0Reg ( 0x2712 )
...
Eth0UdpStreamPortReg::Eth0UdpStreamPortReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool Eth0UdpStreamPortReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x24e) return false; //init value; on false, do not update aByteOffset
...
bool Eth0UdpStreamPortReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x24e) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void Eth0UdpStreamPortReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void Eth0UdpStreamPortReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_Eth0UdpStreamPortReg" );
    ...
    aLogFormat . LogDecValue( aStreamRef, its0UdpStreamPort );
