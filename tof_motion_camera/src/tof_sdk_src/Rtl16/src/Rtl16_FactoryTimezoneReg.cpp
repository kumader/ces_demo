...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class FactoryTimezoneReg 
...
FactoryTimezoneReg::FactoryTimezoneReg ( ) 
: its0Addr ( 0x3b )
, its0Reg ( 0 )
...
FactoryTimezoneReg::FactoryTimezoneReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool FactoryTimezoneReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x3b) return false; //init value; on false, do not update aByteOffset
...
bool FactoryTimezoneReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x3b) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void FactoryTimezoneReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void FactoryTimezoneReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_FactoryTimezoneReg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "ProductionTimezone" );
    aLogFormat . LogDecValue( aStreamRef, its0ProductionTimezone );
