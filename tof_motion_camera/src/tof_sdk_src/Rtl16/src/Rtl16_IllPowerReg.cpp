...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class IllPowerReg 
...
IllPowerReg::IllPowerReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool IllPowerReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x159) return false; //init value; on false, do not update aByteOffset
...
bool IllPowerReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x159) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void IllPowerReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void IllPowerReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_IllPowerReg" );
    ...
    aLogFormat . LogDecValue( aStreamRef, its0IllPower_percent );
