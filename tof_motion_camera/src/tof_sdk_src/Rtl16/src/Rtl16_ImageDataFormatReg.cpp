...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class ImageDataFormatReg 
...
ImageDataFormatReg::ImageDataFormatReg ( ) 
: its0Addr ( 0x4 )
, its0Reg ( 0 )
...
ImageDataFormatReg::ImageDataFormatReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool ImageDataFormatReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x4) return false; //init value; on false, do not update aByteOffset
...
bool ImageDataFormatReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x4) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void ImageDataFormatReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void ImageDataFormatReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_ImageDataFormatReg" );
    ...
    aLogFormat . LogDecValue( aStreamRef, its3ImageDataFormat );
