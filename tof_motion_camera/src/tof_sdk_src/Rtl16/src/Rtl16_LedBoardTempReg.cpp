...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class LedBoardTempReg 
...
LedBoardTempReg::LedBoardTempReg ( ) 
: its0Addr ( 0x1b )
, its0Reg ( 0 )
...
LedBoardTempReg::LedBoardTempReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool LedBoardTempReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x1b) return false; //init value; on false, do not update aByteOffset
...
bool LedBoardTempReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x1b) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void LedBoardTempReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void LedBoardTempReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_LedBoardTempReg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "LedBoardTemp_deg100" );
    aLogFormat . LogDecValue( aStreamRef, its0LedBoardTemp_deg100 );
