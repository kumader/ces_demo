...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class MainBoardTempReg 
...
MainBoardTempReg::MainBoardTempReg ( ) 
: its0Addr ( 0x1c )
, its0Reg ( 0 )
...
MainBoardTempReg::MainBoardTempReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool MainBoardTempReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x1c) return false; //init value; on false, do not update aByteOffset
...
bool MainBoardTempReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x1c) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void MainBoardTempReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void MainBoardTempReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_MainBoardTempReg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "MainBoardTemp_deg100" );
    aLogFormat . LogDecValue( aStreamRef, its0MainBoardTemp_deg100 );
