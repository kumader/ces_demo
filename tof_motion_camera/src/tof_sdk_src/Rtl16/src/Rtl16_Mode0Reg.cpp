...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class Mode0Reg 
...
Mode0Reg::Mode0Reg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool Mode0Reg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x1) return false; //init value; on false, do not update aByteOffset
...
bool Mode0Reg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x1) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void Mode0Reg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void Mode0Reg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_Mode0Reg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "ManualTrigger" );
    aLogFormat . LogDecValue( aStreamRef, its4ManualTrigger );
    aLogFormat . LogElement( aStreamRef, "ClearStatusErrorBits" );
    aLogFormat . LogDecValue( aStreamRef, its6ClearStatusErrorBits );
