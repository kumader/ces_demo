...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class ProcessorStatusReg 
...
ProcessorStatusReg::ProcessorStatusReg ( ) 
: its0Addr ( 0x46 )
, its0Reg ( 0 )
...
ProcessorStatusReg::ProcessorStatusReg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool ProcessorStatusReg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{

    //0Addr  uint2 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint2) > aBufferSize ) return false;
     if (its0Addr != 0x46) return false; //init value; on false, do not update aByteOffset
...
bool ProcessorStatusReg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //0Addr  uint2 serialization with posix endianess
    {
     ...
     if (tmp0Addr != 0x46) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void ProcessorStatusReg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void ProcessorStatusReg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_ProcessorStatusReg" );
    ...
    aLogFormat . LogDecValue( aStreamRef, its0ProcessorTemp_deg );
    aLogFormat . LogElement( aStreamRef, "ProcessorSpeed_10MHz" );
    aLogFormat . LogDecValue( aStreamRef, its8ProcessorSpeed_10MHz );
