...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class Tx_FRAMECOUNT_Reg 
...
//------------------------------ Includes ------------------------------
#include "stdafx.h"
#include "Rtl16_Tx_FRAMECOUNT_Reg.h"
...
Tx_FRAMECOUNT_Reg::Tx_FRAMECOUNT_Reg ( ) 
: its0Addr ( 0x101a )
, its0Reg ( 0x0000 )
...
Tx_FRAMECOUNT_Reg::Tx_FRAMECOUNT_Reg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool Tx_FRAMECOUNT_Reg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{
...
bool Tx_FRAMECOUNT_Reg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
...
//------------------------------ Logging ------------------------------
void Tx_FRAMECOUNT_Reg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void Tx_FRAMECOUNT_Reg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_Tx_FRAMECOUNT_Reg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "Tx_FRAMECOUNT" );
    aLogFormat . LogDecValue( aStreamRef, its0Tx_FRAMECOUNT );
