...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class Tx_IDLETIMEy_Reg 
...
Tx_IDLETIMEy_Reg::Tx_IDLETIMEy_Reg ( ) 
: its0Addr ( 0x1014 )
, its0Reg ( 0 )
...
Tx_IDLETIMEy_Reg::Tx_IDLETIMEy_Reg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool Tx_IDLETIMEy_Reg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{
...
bool Tx_IDLETIMEy_Reg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
...
//------------------------------ Logging ------------------------------
void Tx_IDLETIMEy_Reg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void Tx_IDLETIMEy_Reg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_Tx_IDLETIMEy_Reg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "IDLETIME_tmix" );
    aLogFormat . LogDecValue( aStreamRef, its0IDLETIME_tmix );
