...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class Tx_Py_PREHEAT_Reg 
...
//------------------------------ Includes ------------------------------
#include "stdafx.h"
#include "Rtl16_Tx_Py_PREHEAT_Reg.h"
...
Tx_Py_PREHEAT_Reg::Tx_Py_PREHEAT_Reg ( ) 
: its0Addr ( 0x102a )
, its0Reg ( 0 )
...
Tx_Py_PREHEAT_Reg::Tx_Py_PREHEAT_Reg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool Tx_Py_PREHEAT_Reg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{
...
bool Tx_Py_PREHEAT_Reg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
...
//------------------------------ Logging ------------------------------
void Tx_Py_PREHEAT_Reg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void Tx_Py_PREHEAT_Reg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_Tx_Py_PREHEAT_Reg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "Tx_Py_PREHEAT_tmix" );
    aLogFormat . LogDecValue( aStreamRef, its0Tx_Py_PREHEAT_tmix );
