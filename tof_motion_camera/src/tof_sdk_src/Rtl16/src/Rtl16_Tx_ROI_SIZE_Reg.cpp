...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class Tx_ROI_SIZE_Reg 
...
Tx_ROI_SIZE_Reg::Tx_ROI_SIZE_Reg ( ) 
: its0Addr ( 0x1022 )
, its0Reg ( 0xf014 )
...
Tx_ROI_SIZE_Reg::Tx_ROI_SIZE_Reg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool Tx_ROI_SIZE_Reg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{
...
bool Tx_ROI_SIZE_Reg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
...
//------------------------------ Logging ------------------------------
void Tx_ROI_SIZE_Reg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void Tx_ROI_SIZE_Reg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_Tx_ROI_SIZE_Reg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "Tx_ROI_SIZE_X" );
    aLogFormat . LogDecValue( aStreamRef, its0Tx_ROI_SIZE_X );
    aLogFormat . LogElement( aStreamRef, "Tx_ROI_SIZE_Y" );
    aLogFormat . LogDecValue( aStreamRef, its8Tx_ROI_SIZE_Y );
