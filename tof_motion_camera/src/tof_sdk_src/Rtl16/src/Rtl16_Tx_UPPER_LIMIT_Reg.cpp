...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class Tx_UPPER_LIMIT_Reg 
...
//------------------------------ Includes ------------------------------
#include "stdafx.h"
#include "Rtl16_Tx_UPPER_LIMIT_Reg.h"
...
Tx_UPPER_LIMIT_Reg::Tx_UPPER_LIMIT_Reg ( ) 
: its0Addr ( 0x101c )
, its0Reg ( 0x0ccc )
...
Tx_UPPER_LIMIT_Reg::Tx_UPPER_LIMIT_Reg ( const uint2 & a0AddrValue, const uint2& a0RegValue ) 
...
bool Tx_UPPER_LIMIT_Reg::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{
...
bool Tx_UPPER_LIMIT_Reg::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
...
//------------------------------ Logging ------------------------------
void Tx_UPPER_LIMIT_Reg::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void Tx_UPPER_LIMIT_Reg::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "Rtl16_Tx_UPPER_LIMIT_Reg" );
    aLogFormat . LogElement( aStreamRef, "Addr" );
    aLogFormat . LogHexValue( aStreamRef, its0Addr );
    aLogFormat . LogElement( aStreamRef, "Tx_UPPER_LIMIT" );
    aLogFormat . LogDecValue( aStreamRef, its0Tx_UPPER_LIMIT );
