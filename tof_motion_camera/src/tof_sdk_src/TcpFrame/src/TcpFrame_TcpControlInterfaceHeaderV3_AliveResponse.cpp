///////////////////////////////////////////////////////////////////////////
//     PROJECT: Cpp -- TcpControlInterfaceHeaderV3_AliveResponse
//   COPYRIGHT: R. Hranitzky
//    $RCSfile$
//   $Revision: 4479 $
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class TcpControlInterfaceHeaderV3_AliveResponse 
//              used /usr/local/bin/CppClassCoder.pl to create this file
//              created Fri Feb  9 17:10:41 2018
...
//------------------------------ Includes ------------------------------
#include "stdafx.h"
#include "TcpFrame_TcpControlInterfaceHeaderV3_AliveResponse.h"
...
TcpControlInterfaceHeaderV3_AliveResponse::TcpControlInterfaceHeaderV3_AliveResponse ( ) 
: its0Preamble ( 41452 )
...
bool TcpControlInterfaceHeaderV3_AliveResponse::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{
    ...
    {
     if( aByteOffset + sizeof(uint1) > aBufferSize ) return false;
     if (its3Command != 0xfe) return false; //init value; on false, do not update aByteOffset
    ...
    //5Status  uint1 serialization with posix endianess
    {
     ...
     if (its5Status == 0 || 
      ...
      its5Status == 253 || 
      its5Status == 254 || 
      its5Status == 255); else { return false; } //array; on false, do not update aByteOffset
    ...
    //8Length  uint4 serialization with posix endianess
    {
     ...
     if (its8Length != 0) return false; //init value; on false, do not update aByteOffset
    ...
    //12HeaderData0  uint1 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint1) > aBufferSize ) return false;
     if (its12HeaderData0 != 0) return false; //init value; on false, do not update aByteOffset

     uint1 * pTmp12HeaderData0 = reinterpret_cast< uint1 * > ( buffer + aByteOffset );
     *pTmp12HeaderData0        = its12HeaderData0; //FIXME: no endianess checked here
     aByteOffset      += sizeof(uint1);
    }

    //13HeaderData1  uint1 serialization with posix endianess
    {
     if( aByteOffset + sizeof(uint1) > aBufferSize ) return false;
     if (its13HeaderData1 != 0) return false; //init value; on false, do not update aByteOffset

     uint1 * pTmp13HeaderData1 = reinterpret_cast< uint1 * > ( buffer + aByteOffset );
     *pTmp13HeaderData1        = its13HeaderData1; //FIXME: no endianess checked here
     aByteOffset      += sizeof(uint1);
    }
...
bool TcpControlInterfaceHeaderV3_AliveResponse::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //3Command  uint1 serialization with posix endianess
    {
     ...
     if (tmp3Command != 0xfe) return false; //init value; on false, do not update aByteOffset
    ...
    //5Status  uint1 serialization with posix endianess
    {
     ...
     if (tmp5Status == 0 || 
      ...
      tmp5Status == 253 || 
      tmp5Status == 254 || 
      tmp5Status == 255); else { return false; } //array; on false, do not update aByteOffset
    ...
    //8Length  uint4 serialization with posix endianess
    {
     ...
     if (tmp8Length != 0) return false; //init value; on false, do not update aByteOffset
    ...
    //12HeaderData0  uint1 serialization with posix endianess
    {
     ...
     const uint1 * pTmp12HeaderData0 = reinterpret_cast< const uint1 * > ( buffer + aByteOffset );
     uint1 tmp12HeaderData0 = *pTmp12HeaderData0; //FIXME: no endianess checked

     if (tmp12HeaderData0 != 0) return false; //init value; on false, do not update aByteOffset

     its12HeaderData0     = tmp12HeaderData0;
     aByteOffset += sizeof(uint1);
    }
    ...
    //13HeaderData1  uint1 serialization with posix endianess
    {
     ...
     const uint1 * pTmp13HeaderData1 = reinterpret_cast< const uint1 * > ( buffer + aByteOffset );
     uint1 tmp13HeaderData1 = *pTmp13HeaderData1; //FIXME: no endianess checked

     if (tmp13HeaderData1 != 0) return false; //init value; on false, do not update aByteOffset

     its13HeaderData1     = tmp13HeaderData1;
     aByteOffset += sizeof(uint1);
    }
...
//------------------------------ Logging ------------------------------
void TcpControlInterfaceHeaderV3_AliveResponse::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void TcpControlInterfaceHeaderV3_AliveResponse::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "TcpFrame_TcpControlInterfaceHeaderV3_AliveResponse" );
    ...
    aLogFormat . LogElement( aStreamRef, "12HeaderData0" );
    aLogFormat . LogDecValue( aStreamRef, its12HeaderData0 );
    aLogFormat . LogElement( aStreamRef, "13HeaderData1" );
    aLogFormat . LogDecValue( aStreamRef, its13HeaderData1 );
