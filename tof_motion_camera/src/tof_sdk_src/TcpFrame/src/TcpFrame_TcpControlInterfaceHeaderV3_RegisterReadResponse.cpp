...
//      AUTHOR: R. Hranitzky
//    LANGUAGE: C++ 
// DESCRIPTION: The implementation for class TcpControlInterfaceHeaderV3_RegisterReadResponse 
...
TcpControlInterfaceHeaderV3_RegisterReadResponse::TcpControlInterfaceHeaderV3_RegisterReadResponse ( ) 
: its0Preamble ( 41452 )
...
bool TcpControlInterfaceHeaderV3_RegisterReadResponse::WriteToBuffer( uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset ) const
{
    ...
    {
     if( aByteOffset + sizeof(uint1) > aBufferSize ) return false;
     if (its3Command != 3) return false; //init value; on false, do not update aByteOffset
...
bool TcpControlInterfaceHeaderV3_RegisterReadResponse::ReadFromBuffer( const uint1 * buffer, uint2 aBufferSize, uint2 & aByteOffset )
{
    ...
    //3Command  uint1 serialization with posix endianess
    {
     ...
     if (tmp3Command != 3) return false; //init value; on false, do not update aByteOffset
...
//------------------------------ Logging ------------------------------
void TcpControlInterfaceHeaderV3_RegisterReadResponse::Log( ostream & aStreamRef ) const
{
   Common::LogFormat tmpLogFormat( 10 );
   Log( aStreamRef, tmpLogFormat );
}

void TcpControlInterfaceHeaderV3_RegisterReadResponse::Log( ostream & aStreamRef, Common::LogFormat & aLogFormat ) const
{
   aLogFormat . LogBeginStruct( aStreamRef, "TcpFrame_TcpControlInterfaceHeaderV3_RegisterReadResponse" );
